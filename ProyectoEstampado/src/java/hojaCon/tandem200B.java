...
public class tandem200B {
    ...
    public void prensaTandem(Window comodin, final Listbox procesosMaq) throws SQLException, InterruptedException {
        ...
        myTdm = new Window("Formato Tandem 200 (Tipo B)", "normal", false);
        myTdm.setHeight("600px");
        myTdm.setWidth("370px");
        win.getChildren().clear();
        myTdm.getChildren().clear();

        win.appendChild(myTdm);
        ...
        Label LongPer = new Label("Longitud de Perno de C.");
        Div cen = new Div();
        Label ret = new Label("Kg.f/cm^2");
        final Textbox Lper = new Textbox();
        cen.appendChild(Lper);
        cen.appendChild(ret);

        row13.appendChild(LongPer);
        row13.appendChild(cen);

        Label PresC = new Label("Presión de Colchón");
        Div centr = new Div();
        ...
        Label AcEnb = new Label("Aceite Embutido");
        final Textbox Ace = new Textbox();
        row15.appendChild(AcEnb);
        row15.appendChild(Ace);

        Label PersEst = new Label("Personal de Estampado");
        ...
        Label numPren = new Label("Selecciona una prensa");
        final Listbox numPrenLi = new Listbox();
        numPrenLi.setMold("select");
        Listitem num1 = new Listitem("TDM 200 (B-01)");
        Listitem num2 = new Listitem("TDM 200 (B-02)");
        Listitem num3 = new Listitem("TDM 200 (B-03)");
        numPrenLi.appendChild(num1);
        numPrenLi.appendChild(num2);
        numPrenLi.appendChild(num3);
        row24.appendChild(numPren);
        row24.appendChild(numPrenLi);
        ...
        row25.appendChild(observac);
        row25.appendChild(obs);
                ...
                try {
                    if (media != null) {
                        ...
                        if (media instanceof org.zkoss.image.Image) {
                            ...
                            OutputStream outputStream = new FileOutputStream(new File("C:/Documents and Settings/alejandro.montes/My Documents/NetBeansProjects/ProyectoEstampado/web/img/HojaCond/TD200A/" + dire));
                            ...
                            File aud = new File("C:/Documents and Settings/alejandro.montes/My Documents/NetBeansProjects/ProyectoEstampado/web/img/HojaCond/TD200A/" + dire);
        ...
        btnAc.addEventListener(Events.ON_CLICK, new org.zkoss.zk.ui.event.EventListener() {
            ...
            public void onEvent(org.zkoss.zk.ui.event.Event event) throws Exception {
                ...
                boolean x = true;
                //Tandem 200A utiliza el número de máquina número 1
                int numeroMaq = 4;
                //Bl200 utiliza el formato número 9
                int numeroFormato = 15;
                ConnSql conectar = new ConnSql();
                ...
                String PersonalEstam = PerstEsta.getValue();
                double alturaTroq = Double.valueOf(altTroqEn.getValue());


                String presionDeColchon = PCol.getValue();
                String AceiteEmbutido = Ace.getValue();
                String SujecA = SujAr.getValue();
                String SujeAb = SujAba.getValue();
                String Mater = mate.getValue();
                String longitudPerno = Lper.getValue();
                String prensa = numPrenLi.getSelectedItem().getLabel();
                String observa = obs.getValue();


                int idMat = conectar.obtenerIdMaterial(Mater);
                try {
                    conectar.eliminarDuplicidadTD200B(idHC, sec);
                    conectar.altaTandem200B(numeroMaq, numeroFormato, idp, idHC, idTroq, idDepa, sec, idMat, presionB, alturaTroq, presionDeColchon, AceiteEmbutido, SujecA, SujeAb, longitudPerno, PersonalEstam, observa, prensa);
                } catch (Exception ex) {
        ...
        myTdm.appendChild(myGrid);
        myTdm.appendChild(btnG);
        myTdm.setSizable(true);
        myTdm.setMaximizable(true);
        myTdm.setContentStyle("overflow:auto");
        myTdm.doModal();
